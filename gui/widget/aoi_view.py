# Vendored fork of pysepal 3.6.1 pysepal/solara/components/aoi/aoi_view.py.
# Adds restore-on-load (method/admin/draw) + ASSET capture. Re-diff on any pysepal bump.
...
from gui.widget.asset_select import AssetSelectComponent
...
def AoiView(
    ...
    restore_asset: Optional[Dict] = None,
    on_selection: Optional[Callable[[Optional[Dict]], None]] = None,
):
    ...
    # The method we restored on load. on_method_change skips its clear while the
    # selected method equals this, so restore (and reacton's double effect-run)
    ...
    def _seed_draw_control(gdf):
        # Best-effort: re-populate the editable DrawControl from a loaded gdf so a
        # restored DRAW AOI is editable. The geometry is shown regardless by the
        # shell's show_aoi_on_map, so a failure here is cosmetic.
    ...
    def _apply_restore():
        result = reactive_value.value  # the loaded AoiResult (app set it before mount)
        ...
        # ASSET: the vendored AssetSelectComponent seeds from restore_asset.
