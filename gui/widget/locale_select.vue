...
<script>
// App-owned copy of pysepal's LocaleSelect.vue. The ONLY functional change
// versus upstream (pysepal 3.6.2) is how the chosen locale is pushed back to
// Python: upstream uses `this.$emit("update:selected_locale", code)`, the Vue
// `.sync` convention that requires a parent component bound with
// `:selected_locale.sync`. As a standalone root widget there is no such
// parent, so the emit is dropped and the Python trait never updates. Here we
// assign the synced trait directly (`this.selected_locale = code`), which is
// how every other pysepal widget (e.g. FileInput.vue) reports values to
// Python — this makes the `_on_locale_select` observer fire and persist the
// choice to ~/.sepal-ui-config.
export default {
      ...
      handler(newCountries) {
        ...
        if (newCountries.length === 1) {
          ...
          if (this.currentLanguage !== onlyLanguage) {
            ...
            this.selected_locale = onlyLanguage;
          }
        }
      },
    },
  ...
  mounted() {
    ...
    if (localeExists) {
      ...
      // Push the corrected value back to Python
      this.selected_locale = this.available_locales[0].code;
    } else {
      // Fallback to 'en' if no locales available
      const fallbackLocale = "en";
      this.currentLanguage = fallbackLocale;
      this.selectedLanguage = fallbackLocale;
      this.selected_locale = fallbackLocale;
    }
  },
  ...
  methods: {
    ...
    selectLanguage(code) {
      ...
      if (this.selectedLanguage !== this.currentLanguage) {
        ...
        // Push the choice to Python via direct trait assignment so the
        // `_on_locale_select` observer fires and persists it to config.
        this.selected_locale = this.selectedLanguage;
      }
