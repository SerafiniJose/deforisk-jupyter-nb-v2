...
def test_output_files_tolerate_missing_stats():
    """output_files() gracefully handles None stats and None individual paths."""
    # Case 1: stats is None — should return base class list if model_path/
    # samples_path are set; otherwise empty.
    m_none = MWModel(name="m")
    files_none = m_none.output_files()
    assert isinstance(files_none, list)
    assert len(files_none) == 0  # No model_path or samples_path set on bare model

    j_none = JNRBenchmarkModel(name="j")
    files_j_none = j_none.output_files()
    assert isinstance(files_j_none, list)
    assert len(files_j_none) == 0  # No model_path or samples_path set on bare model

    # Case 2: stats set but individual paths are None
    m_stats_empty = MWModel(name="m", stats=MWStats())
    files_empty = m_stats_empty.output_files()
    assert isinstance(files_empty, list)
    assert len(files_empty) == 0  # Only base class (empty), no artifact paths

    j_stats_empty = JNRBenchmarkModel(name="j", stats=JNRStats())
    files_j_empty = j_stats_empty.output_files()
    assert isinstance(files_j_empty, list)
    assert len(files_j_empty) == 0  # Only base class (empty), no artifact paths


def test_jnr_defrate_files_are_deliberately_excluded(tmp_path):
    """JNR's apply()-time defrate tables are NOT in output_files() for deletion safety.

    This is the critical exclusion contract: deleting a model must not remove
    files a prediction still references. Defrate CSVs from apply() belong to
    predictions, not to the model itself.
    """
    j = JNRBenchmarkModel(
        name="j",
        defrate_files={"2020": tmp_path / "defrate_2020.csv"},
        stats=JNRStats(
            tab_dist_path=tmp_path / "tab_dist.csv",
            perc_dist_png=tmp_path / "perc_dist_p.png",
        ),
    ...
    files = j.output_files()
    # Fit-time artifacts SHOULD be included
    assert tmp_path / "tab_dist.csv" in files
    assert tmp_path / "perc_dist_p.png" in files
    # Apply-time defrate CSVs MUST NOT be included (deletion safety)
    assert tmp_path / "defrate_2020.csv" not in files
